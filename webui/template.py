...
<body>
        ...
        <div id="bufferTab" class="tab-content">
            <div class="search-section">
                <div class="search-box">
                    ...
                    <button class="btn btn-danger" onclick="clearBuffer()">🗑️ Clear Buffer</button>
                </div>
            </div>
    ...
    <script>
        ...
        async function loadBuffer() { try { const resp = await fetch('/api/buffer'); const data = await resp.json(); const listDiv = document.getElementById('bufferList'); listDiv.innerHTML = ''; if (data.count > 0) { data.messages.forEach(item => { const card = document.createElement('div'); card.className = 'memory-card'; card.innerHTML = '<div class="memory-header"><span class="memory-role">Buffer</span><span class="memory-score">' + (item.scope_key || 'global') + '</span></div><div class="memory-content">' + escapeHtml(item.content || '') + '</div><div class="memory-meta"><span class="meta-tag"><span class="meta-tag-icon">📅</span>' + (item.timestamp ? new Date(item.timestamp * 1000).toLocaleString() : 'N/A') + '</span></div>'; listDiv.appendChild(card); }); } else { listDiv.innerHTML = '<div class="state-message"><div class="state-icon">📭</div><div class="state-text">缓冲区为空</div></div>'; } } catch (e) { alert('Load failed: ' + e.message); } }
        async function clearBuffer() { if (!confirm('确定要清空缓冲区吗？')) return; try { const resp = await fetch('/api/buffer/clear', {method: 'POST'}); const data = await resp.json(); alert('已清空 ' + data.cleared + ' 条缓冲区消息'); loadBuffer(); } catch (e) { alert('Clear failed: ' + e.message); } }
